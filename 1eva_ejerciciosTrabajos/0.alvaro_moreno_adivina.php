	...
	//me envia: //header("Location:0.alvaro_moreno_adivina.php?primera&opcion=$opcion&intentos=$intentos&aleatorio=$aleatorio");
	...
	if($intentos==5 && !$acierto){
		...
		echo "<a href='0.alvaro_moreno_adivina.php.php'>Jugar otra vez</a>";
	} else{
		if(!isset($_GET['primera'])){
			echo "<p>A jugar!!!</p>";
		}else{
	...
	if ($intentos<5 && !$acierto){//Muestro el formulario condicionado a seguir jugando a: tener intentos y no haber acertado
		echo "<form name='formulario' method='get' action='0.alvaro_moreno_procesa.php'>
